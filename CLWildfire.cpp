#include "CLWildfire.hpp"

// constructeur de la classe OpenCL
CLWildfire::CLWildfire(int width, int height, int nbPoint) {
	...
	img = cv::Mat(cv::Size(width,height), CV_8UC3,cv::Scalar(70,141,189));
	// générer les points initiaux
	CLWildfire::generateDot(nbPoint);
	...
	randomSeed = cl::Buffer(context, // contexte du buffer 
						CL_MEM_READ_ONLY, // mode
						img.channels()*sizeof(int) // taille
						);
	// instancier la fenêtre d'affichage
	cv::namedWindow("Feu de forêt",CV_WINDOW_KEEPRATIO );
	// afficher l'image dans la fenêtre
	cv::imshow("Feu de forêt", img);
}

void CLWildfire::run(int ms) {
	int key;
	//int i = 1,it=501;
	//clock_t stopTime;
	...
	while(true) {
		...
		execKernel();
		readMemory();
		//if (i == 1 or i == 2 or i == 3 or i == 5 or i == 10 or i == 25 or i == 50 or i == 100 or i == 250 or i == 500) {
			//std::cout<<i<<" : "<<double((clock()-startTime)*1e-6)<<std::endl;
		//}
		//i++;
	}
...
void CLWildfire::generateDot(int nbPoint) {
	// graine
	...
	for (int i=0;i<nbPoint;i++) {
		...
		img.data[position] = 0;
		img.data[position+1] = 238;
		img.data[position+2] = 174;
	}
}


void CLWildfire::makeKernel(void) {
	// ouvrir le fichier source OpenCL
	std::ifstream sourceFile("wildfireKernel.cl");
	// lire fichier source
	...
	// Créer le kernel
	kernel = cl::Kernel(program, "wildfire");
}

void CLWildfire::writeMemory(void) {
	// graine
	srand((time(NULL)+rand())%(img.total()+rand()));
	int * seed = new int[img.channels()];
	int modulo = 100;
	for (int i=0;i<img.channels();i++) {
		seed[i] = (rand()%modulo)+1;
		modulo *= 10;
	}
	...
	// attendre la fin du chargement du buffer dans la queue
	event.wait();
	queue.enqueueWriteBuffer(randomSeed,  // buffer de copie
						CL_TRUE, // bloquer en écriture
						0, // offset de départ
						img.channels()*sizeof(int), // taille 
						seed, // contenu du buffer
						NULL, // vecteur d'événement, par défaut null
						&event // variable d'événement
						);
	// attendre la fin du chargement du buffer dans la queue
	...
	kernel.setArg(3, randomSeed);
	kernel.setArg(4, outputImg);
}

void CLWildfire::readMemory(void) {
	queue.enqueueReadBuffer(outputImg, CL_TRUE, 0, img.total() * img.channels() * sizeof(uchar), img.data);
	cv::imshow("Feu de forêt", img);
}

void CLWildfire::execKernel(void) {
	// exécuter le kernel
...
CLWildfire::~CLWildfire(void) {
	queue.finish();
	queue.flush();
	cv::destroyWindow("Feu de forêt");
	std::cout<< "byebye wildfire"<<std::endl;
}
