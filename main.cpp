...
// Contenu     : Automate cellulaire : feu de forêt             //
...
std::string usage(std::string appName) {
	return	"\nusage : "+appName+" largeur hauteur nbArbres millisec\n";
}

std::string man(void) {
	return	"\n* largeur  : largeur en pixel de la fenêtre\n"
		"* hauteur  : hauteur en pixel de la fenêtre\n"
		"* nbPoints : nombre d'arbres à afficher\n"
...
int main(int argc, char *argv[]){
	...
	try {
		...
		CLWildfire wildfire(atoi(argv[1]),atoi(argv[2]),atoi(argv[3]));
		wildfire.makeKernel();
		wildfire.run(atoi(argv[4]));
	} catch (cl::Error err) {
